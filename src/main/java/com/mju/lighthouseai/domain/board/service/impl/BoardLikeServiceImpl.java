...
import com.mju.lighthouseai.domain.board.exception.BoardLikeErrorCode;
...
public class BoardLikeServiceImpl implements BoardLikeService {
    ...
    public void addBoardLike(
        ...
        Board board = boardRepository.findById(boardLikeServiceRequestDto.board().getId())
                .orElseThrow(() -> new NotFoundBoardException(BoardLikeErrorCode.NOT_FOUND_Board));
        User user = userRepository.findById(boardLikeServiceRequestDto.user().getId())
                .orElseThrow(() -> new NotFoundBoardException(BoardLikeErrorCode.NOT_FOUND_User));

        userId = user.getId();//boardLikeServiceRequestDto.user().getId();
        boardId = board.getId(); // boardId = boardLikeServiceRequestDto.board().getId();
